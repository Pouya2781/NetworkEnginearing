...
const ForwardRumor = ({ image, firstName, lastName, loadImage, timeStamp, score, scoreCount, messageDataId, message, setSelectedChats, chatId }) => {
  ...
  const handleRumorClick = () => {
    if (!isSelected) {
        setIsSelected(true);
        setSelectedChats(prev => [ ...prev, { chatId: chatId, chatType: "rumor"}]);
    } else {
        setIsSelected(false);
        setSelectedChats(prev => prev.filter((value) => !(value.chatId == chatId && value.chatType == "rumor")));
    }
  }
  ...
  return (
    <div onClick={handleRumorClick} className="overflow-hidden pt-2 cursor-pointer">
      <div className="flex justify-end bg-white border-b">
        ...
        <div className="flex justify-end items-center h-[5rem] w-[9rem] text-gray-400">
