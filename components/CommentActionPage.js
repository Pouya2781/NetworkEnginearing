...
const CommentActionPage = ({ className, messageDataId, userId, firstName, lastName, setIsCommentActionOpen }) => {
  ...
  const handleClickSend = async () => {
    ...
    formData.append('chatId', json.data.id);
    formData.append('chatType', "rumor");
    if (message.message.length > 0)
      formData.append('message', message.message);
    formData.append('image', imageInput.current.files[0]);

    res = await fetch('http://localhost:3001/api/user/message', {
                method: 'POST',
                headers: {
                    'x-auth-token': window.localStorage.getItem('messenger_auth_token')
                },
        ...
        <div className="flex-col justify-center items-start bg-gray-300 w-[39rem] h-[37rem] rounded-[1rem]">
            ...
            <div className="ml-[1rem] -mt-[3.75rem] relative flex items-center">
              <div className="pl-2 items-center flex h-[3.1rem]">
                <p className="font-bold text-[18px] text-black/90">Send a comment...</p>
              </div>
...
export default CommentActionPage;
